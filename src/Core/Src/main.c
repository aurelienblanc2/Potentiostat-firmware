...
static void DisconnectUsbPort(void)
{
	GPIO_InitTypeDef GPIO_InitStruct = {0};

	/*Configure GPIO pins : USB_DM_Pin PA11 USB_DP_Pin */
...
static void Potentiostat_Init(void)
{
	  system_config *cfg = GetSystemConfig();

	  LoadDataConfig(cfg, LOAD_FROM_NVM | LOAD_IFDEF_SAVE);
	  DAC_Init(cfg->p_dac_cfg);
	  Analog_Init(cfg->p_ancfg);
	 // PID_Init(&cfg->pid_cfg, &GetAnalogData(eANCH_WEOUT)->mean);
	  PID_Init(&cfg->pid_cfg, &GetPotentiostatParam()->adc.weout);
	  BlinkLed_Init();
	  QueryListInit();
	  ModbusInit();
	  Humidity_Sensor_Init(&hi2c2);

	  task_timer_create(&p_dtm_task,
			  GetDatetimeFromIRTC,
			  GetSysDateTime(),
			  TASK_TIMER_SEC(1));

	  task_timer_start(&p_dtm_task, TMR_CONTINUOUS);
}
...
void MX_ADC5_EIS_Init(void)
{
  ...
  */
  hadc5.Instance = ADC5;
  hadc5.Init.ClockPrescaler = ADC_CLOCK_SYNC_PCLK_DIV1;
  hadc5.Init.Resolution = ADC_RESOLUTION_12B;
  hadc5.Init.DataAlign = ADC_DATAALIGN_RIGHT;
  hadc5.Init.GainCompensation = 0;
  hadc5.Init.ScanConvMode = ADC_SCAN_ENABLE;
...
static void MX_DMA_Init(void)
{
  ...
  hdma_memtomem_dma1_channel7.Init.Priority = DMA_PRIORITY_HIGH;
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, IVG2_Pin|IVG3_Pin|IVG4_Pin|BLUE_LED_Pin
                          |IVG1_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pins : IVG2_Pin IVG3_Pin IVG4_Pin CE_EN_Pin
                           IVG1_Pin */
  GPIO_InitStruct.Pin = IVG2_Pin|IVG3_Pin|IVG4_Pin|CE_EN_Pin
                          |IVG1_Pin;
